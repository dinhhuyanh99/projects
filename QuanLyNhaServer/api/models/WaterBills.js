...
var WaterBillSchema = new Schema({
	billId: {
...
WaterBillSchema.pre('save', function(next){
	this.contigentNumber = this.get('currentNumber') - this.get('prevNumber');
	this.totalBill = this.get('contigentNumber') * this.get('overallWaterFee');
	next();
});



module.exports = mongoose.model('WaterBills', WaterBillSchema);